...
public final class MultiMap<K, V> {
    ...
    public void saveAdditional(ListTag li, ReikaNBTHelper.NBTIO<K> converterK, ReikaNBTHelper.NBTIO<V> converterV) {
        for (Entry<K, Collection<V>> e : data.entrySet()) {
            CompoundTag entry = new CompoundTag();
            entry.put("key", ReikaNBTHelper.getTagForObject(e.getKey(), converterK));
            ReikaNBTHelper.writeCollectionToNBT(e.getValue(), entry, "values", converterV);
            li.add(entry);
        }
    }

    public void load(ListTag li, ReikaNBTHelper.NBTIO<K> converterK, ReikaNBTHelper.NBTIO<V> converterV) {
        this.clear();
