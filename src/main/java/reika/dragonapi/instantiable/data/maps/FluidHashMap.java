...
public final class FluidHashMap<V> {
    ...
    public FluidHashMap<V> setOneWay(Matcher<V> m) {
        oneWay = true;
        this.matcher = m;
        return this;
    }
...
//todo        ReikaFluidHelper.sortFluids(sorted);
    }

    private V putKey(FluidStack is, V value) {
        ThresholdMapping<V> map = this.data.get(is.getFluid().toString());
        if (map == null) {
            map = new ThresholdMapping();
            this.data.put(is.getFluid().toString(), map); //todo get name not toString
        }
        return map.addMapping(is.getAmount(), value);
    }

    public V get(FluidStack is) {
        ThresholdMapping<V> map = this.data.get(is.getFluid().toString()); //todo get name not toString
        if (map != null) {
    ...
    public V getForValue(FluidStack is) {
        ThresholdMapping<V> map = this.data.get(is.getFluid().toString()); //todo get name not toString
        if (map != null) {
            V ret = map.getForValue(is.getAmount(), false);
            return ret;
        }
        return null;
    }
    ...
    public V put(FluidStack is, V value) {
        if (oneWay && data.containsKey(is.getFluid().toString())) { //todo get name not toString
            if (matcher != null) {
                V v = this.get(is);
    ...
    public boolean containsKey(Fluid f) {
        return data.containsKey(f.toString());
    }
...
//                todo li.add(new FluidStack(FluidRegistry.getFluid(fs), (int)s));
            }
        }
        return li;
    }
...
//            todo Fluid f = FluidRegistry.getFluid(fs);
//            sb.append(f.toString()+"="+data.get(fs)); //todo was ReikaFluidHelper.fluidToStrong(f)
//            sb.append(",\n");
        }
        sb.append("}\n");
        return sb.toString();
    ...
    private V removeKey(FluidStack is) {
        ThresholdMapping<V> map = this.data.get(is.getFluid().toString()); //todo get name not toString
        return map != null ? map.remove(is.getAmount()) : null;
    }
