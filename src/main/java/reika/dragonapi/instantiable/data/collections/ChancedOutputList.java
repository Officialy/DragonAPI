...
public final class ChancedOutputList {
    ...
    public float getItemChance(ItemStack is) {
        ...
        return get != null ? allowOverflow ? get.floatValue() : Mth.clamp_float(get.floatValue(), 0, 100) : 0;
    }
...
/*
	public void multiplyChances(float f) {
		...
		for (ItemStack is : data.keySet()) {
			float f2 = Mth.clamp(f*data.get(is), 0, 100);
			data.put(is, f2);
		}
	}*/
	...
	public void powerChances(double p) {
		...
		for (ItemStack is : data.keySet()) {
			...
			float next = Mth.clamp(pow, 0, 100);
			data.put(is, next);
		}
	}*/
