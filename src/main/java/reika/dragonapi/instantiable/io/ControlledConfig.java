...
public class ControlledConfig {
    ...
    private static int genUserHash() {
		...
		return h1 ^ h2;
		 */
        return FMLEnvironment.getDist() == Dist.CLIENT ? getClientUserHash() : 0;
    }
    ...
    protected final void resetConfigFile() {
        ...
        if (backup.exists())
            backup.delete();
        try {
            DragonAPI.LOGGER.info(configMod.getDisplayName().toUpperCase()+": Writing Backup File to "+path);
            DragonAPI.LOGGER.info(configMod.getDisplayName().toUpperCase()+": Use this to restore custom IDs if necessary.");
            backup.createNewFile();
            if (!backup.exists())
                DragonAPI.LOGGER.error(configMod.getDisplayName().toUpperCase()+": Could not create backup file at "+path+"!");
            else {
            ...
            DragonAPI.LOGGER.error(configMod.getDisplayName().toUpperCase()+": Could not create backup file due to IOException!");
            e.printStackTrace();
        }
        configFile.delete();
    }

    private void versionCheck() { //pre init used to be here
    ...
    public void loadCustomConfigFile(String file) { //pre init used to be here
    ...
    public void loadSubfolderedConfigFile() { //pre init used to be here
        ...
        String file = FMLPaths.GAMEDIR.get()+"/"+"config"+"/"+author+"/"+name+".cfg";
        this.loadConfigFile(new File(file));
    }
        ...
        for (ConfigList cfg : specialFiles.keySet()) {
            ...
            if (s == null) {
                ...
                try {
                    ...
                    configMod.getModLogger().error("Could not parse config value for "+this.getLabel(cfg));
                }
