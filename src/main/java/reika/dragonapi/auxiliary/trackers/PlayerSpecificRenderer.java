...
public final class PlayerSpecificRenderer {
   ...
   private final ReikaModel modelReika;
   //private final SamakiModel modelSamaki = new SamakiModel();

   private PlayerSpecificRenderer() {
       modelReika = new ReikaModel(ReikaModel.createBodyLayer().bakeRoot());
   ...
   public void registerIntercept() {
       // This is no longer valid in 1.20.1, player renderers are replaced via events
   ...
   */
   ...
   public void registerRenderer(UUID uuid, PlayerRenderObj r) {
       ...
       if (/*DragonAPI.isReikasComputer() && */ReikaObfuscationHelper.isDeObfEnvironment()) {
           ...
           renders.addValue(DragonAPI.Officialy_UUID, r);
       }
   }
...
//     todo       ReikaShader.instance.prepareRender(ep);
   ...
   /*
   private static class ModelReikaEars extends PlayerModelRenderer {
...
/*
   private static final class CustomPlayerRenderer extends AvatarRenderer<AbstractClientPlayer> {
       ...
       public CustomPlayerRenderer(EntityRendererProvider.Context p_174557_, boolean p_174558_) {
           ...
           this.addLayer(new reika.dragonapi.instantiable.rendering.GlowLayer(this));
       }
       ...
       private void renderWithoutTextureBind(PoseStack stack, MultiBufferSource buffer, int packedLight, LivingEntity ep, float f1, float f2, float f3, float f4, float f5, float f6) {
           this.getModel().setupAnim((AbstractClientPlayer) ep, f1, f2, f3, f4, f5);
           if (!ep.isInvisible()) {
               // this.getModel().renderToBuffer(stack, consumer, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
               ...
               // Alpha func is gone, handled by shaders.
               // this.getModel().renderToBuffer(stack, consumer, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 0.15F);
...
//            prev = O
       ...
       private void compensateAngles() {
           ...
           if (partialTick == 1.0F) {
               ...
               double par1 = 0; //event.getMouseX() * width / mc.getWindow().getWidth();
               double par2 = 0; //height - event.getMouseY() * height / mc.getWindow().getHeight() - 1;
